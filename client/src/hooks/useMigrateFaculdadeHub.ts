...
export function useMigrateFaculdadeHub() {
  ...
  useEffect(() => {
    // Wait until user is authenticated and Firestore has hydrated
    if (!user) return
    if (localStorage.getItem(MIGRATION_KEY)) return

    const hubSubjects = subjects.filter(s => s.hubId === HUB_ID)
    // Firestore must have loaded before we proceed (expect at least the 15 known subjects)
    if (hubSubjects.length < 15) return
  ...
  }, [user, subjects, updateSubject, addSubject])
}
